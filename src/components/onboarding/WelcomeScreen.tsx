...
import { EyeIcon, EyeOffIcon, MailIcon, KeyIcon, ArrowRightIcon, UserIcon, BriefcaseIcon, ChevronRightIcon } from 'lucide-react';
...
const WelcomeScreen: React.FC<WelcomeScreenProps> = ({
  ...
  return <div className="flex flex-col items-center justify-center min-h-full p-6 bg-gradient-to-b from-blue-50 to-white">
      <div className="w-full max-w-sm">
        ...
        {!loginType && <div className="mb-6 animate-fade-in">
            <p className="text-center text-sm text-gray-600 mb-4">
              Choose your login type
            </p>
            <div className="grid gap-4">
              <button type="button" onClick={() => selectLoginType('employee')} className="flex items-center justify-between p-4 bg-white border border-gray-200 rounded-xl hover:bg-blue-50 hover:border-blue-200 transition-all duration-300 group">
                ...
                <ChevronRightIcon size={20} className="text-gray-400 group-hover:text-blue-500 transform group-hover:translate-x-1 transition-transform" />
              </button>
              <button type="button" onClick={() => selectLoginType('hr')} className="flex items-center justify-between p-4 bg-white border border-gray-200 rounded-xl hover:bg-gradient-to-r hover:from-purple-50 hover:to-indigo-50 hover:border-purple-200 transition-all duration-300 group relative overflow-hidden">
                ...
                <div className="flex items-center relative z-10">
                  <div className="w-10 h-10 rounded-full bg-purple-100 flex items-center justify-center mr-3 group-hover:scale-110 transition-transform duration-300 shadow-sm group-hover:shadow">
                    <BriefcaseIcon size={20} className="text-purple-600 group-hover:text-purple-700" />
                  </div>
                  <div className="text-left">
                ...
                <div className="relative z-10 flex items-center">
                  ...
                  <ChevronRightIcon size={20} className="text-gray-400 group-hover:text-purple-500 transform group-hover:translate-x-1 transition-all duration-300" />
                </div>
              </button>
            </div>
          </div>}
        {/* Login/Signup Form - Only shown after selecting login type */}
        {loginType && <div className="animate-fade-in">
            <div className="flex items-center mb-6">
              <button onClick={() => setLoginType(null)} className="flex items-center text-sm text-gray-600 hover:text-gray-800">
                <ArrowRightIcon size={16} className="transform rotate-180 mr-1" />
                Back
              </button>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button type="submit" className={`w-full py-3 ${loginType === 'hr' ? 'bg-purple-500 hover:bg-purple-600' : 'bg-blue-500 hover:bg-blue-600'} text-white font-medium rounded-xl transition-colors duration-200 flex items-center justify-center`}>
